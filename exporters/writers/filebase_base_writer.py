...
class FilebaseBaseWriter(BaseWriter):
    ...
    def __init__(self, options):
        ...
        if not self.filename:
            ...
            self.options['filebase'] = os.path.sep.join(self.read_option('filebase').split(os.path.sep)[:-1])

    def _get_filename(self):
        return self.read_option('filebase').format(datetime.datetime.now()).split(os.path.sep)[-1]
