...
def _check_valid_options(config_section, section_name):
    if 'name' not in config_section or 'options' not in config_section:
        raise ConfigurationError(
            'Module for section "{}" is missing name and/or option definitions'
            .format(section_name))
    ...
    for option in options:
        _check_required_config_section(option, config_section)


def _check_required_config_section(option_definition, config_section):
...
def _get_module_supported_options(module_name):
    try:
        ...
        base_supported_options = getattr(mod, class_path_list[-1]).base_supported_options
        supported_options = getattr(mod, class_path_list[-1]).supported_options
        supported_options.update(base_supported_options)
